...
class UserService
{
    public function getDashboardSummary(): array
    {
        $counts = User::query()
            ...
            ->selectRaw('SUM(CASE WHEN telephone IS NULL OR telephone = ? OR adresse IS NULL OR adresse = ? THEN 1 ELSE 0 END) as profils_incomplets', ['', ''])
            ->first();
    ...
    {
        if (($data['role'] ?? null) !== RoleEnum::SUPER_ADMIN->value && ($data['role'] ?? null) !== RoleEnum::SUPER_ADMIN) {
            return;
        }
