...
export function ListeEleves({
  ...
  return (
    <section className="panel users-table-panel">
      ...
      {loading ? (
        <div className="screen-state users-table-state">Chargement des élèves...</div>
      ) : eleves.length === 0 ? (
        <div className="screen-state users-table-state">Aucun élève trouvé.</div>
      ) : (
            ...
            <tbody>
              {eleves.map((eleve) => (
                <tr key={eleve.id}>
                  ...
                  <td>
                    {eleve.classes && eleve.classes.length > 0 ? (
                      <span className="badge badge-info" style={{ backgroundColor: 'var(--primary-light)', color: 'var(--primary-dark)' }}>
                        {eleve.classes[0].nom_classe}
                      </span>
                    ) : (
                      <span className="text-muted" style={{ fontSize: '0.85rem' }}>Non affecté</span>
                    )}
                  </td>
                  <td>
                    <span>{eleve.telephone || 'Non renseigné'}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <ActionMenu
                      ...
                      items={[
                        { label: 'Voir', onClick: () => void onShow(eleve.id) },
                        { label: 'Modifier', onClick: () => onEdit(eleve) },
                        { label: 'Affecter à une classe', onClick: () => onInscrire(eleve) },
                        { label: eleve.actif ? 'Désactiver' : 'Activer', onClick: () => void onToggle(eleve.id) },
                        { label: 'Supprimer', onClick: () => void onDelete(eleve.id), danger: true },
                      ]}
                    />
                  </td>
                </tr>
